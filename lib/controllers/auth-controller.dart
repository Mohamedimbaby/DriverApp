...
import 'package:driver_app/models/refresh_token.dart';
import 'package:driver_app/models/reg_api.dart';
import 'package:driver_app/models/login_api.dart';
import 'package:driver_app/services/api-list.dart';
import 'package:driver_app/services/server.dart';
import 'package:driver_app/services/user-service.dart';
import 'package:driver_app/services/validators.dart';
import 'package:driver_app/views/main_screen.dart';
...
class AuthController extends GetxController {
  ...
  loginOnTap({BuildContext? context, String? email, String? pass , deviceToken}) async {
    loader = true;
    Future.delayed(Duration(milliseconds: 10), () {
      update();
    });
    ...
    if (email_validator == null && pass_validator == null) {
      ...
      server
        ...
        if (response != null && response.statusCode == 200) {
         ...
         server
             .putRequest(endPoint: APIList.token! + loginData.data!.id.toString()+deviceToken,).then((response) {
           print(response);
         });
         userService.saveString(
